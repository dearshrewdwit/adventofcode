  ...
  grid.forEach((row, rIndex) => {
    row.split('').forEach((tree, cIndex) => {
      const scenicScore = Object
        .keys(nextCoords)
        .map(direction => findScore(grid, tree, 0, direction, ...nextCoords[direction](rIndex, cIndex)))
        .reduce((a, b) => a * b, 1)
...
const findScore = (grid, tree, score, direction, rIndex, cIndex) => {
  ...
  return findScore(grid, tree, score+1, direction, rIndexNew, cIndexNew)
}
